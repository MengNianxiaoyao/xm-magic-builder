...
export async function insertText(text: string): Promise<boolean> {
    ...
    const success = await editor.edit((builder) => {
        if (plan.replaceLine) {
            builder.replace(editor.document.lineAt(active.line).range, plan.content);
        } else {
...
export interface InsertPlan {
    ...
    character: number;
    content: string;
    /** 为 true 时整行替换（空行占用），否则按位置插入 */
    replaceLine?: boolean;
}

/**
 * 根据光标位置决定插入位置与内容（纯函数，便于测试）：
 * - 空行（含纯空白行）行首且有下一行：直接占用该行，不另起行；
 * - 行首（含末行空行）：原地 `文本 + 换行`，将当前行下推；
...
export function buildInsertPlan(
    ...
    if (isLineStart && lineText.trim() === '' && hasNextLine) {
        return { line: lineIndex, character: 0, content: text, replaceLine: true };
    }
