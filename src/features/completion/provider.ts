...
function createCompletionItem(
    ...
    const item = new vscode.CompletionItem(
        fullText,
        isSubKeyword ? vscode.CompletionItemKind.Keyword : vscode.CompletionItemKind.Value
    );
...
function buildCompletionList(
    ...
    return sortByUsage(items, usage, ({ keyword, c }) => `${keyword}:${c.label}`).map(({ keyword, c }) =>
        createCompletionItem(c, keyword, beforeEq, context, position, eqIndex, usage)
    );
}
    ...
    const provider = vscode.languages.registerCompletionItemProvider(
        'xm',
        {
            provideCompletionItems(document, position) {
                ...
                return new vscode.CompletionList(
                    buildCompletionList(completions, usage, extContext, position, eqIndex, ''),
                    false
                );
            },
        },
